...
module.exports = (io) => {
  ...
  router.post("/add", authmiddle, uploadFields, async (req, res) => {
    try {
      ...
      const attachments = {
        gstCertificate: await handleUpload(req.files?.gstCertificate),
        panCard: await handleUpload(req.files?.panCard),
        cancelledCheque: await handleUpload(req.files?.cancelledCheque),
        msme: await handleUpload(req.files?.msme),
      };
  ...
  router.patch("/status/:id", authmiddle, async (req, res) => {
    ...
    try {
      ...
      io.emit("vendor_updated", updatedItem);
